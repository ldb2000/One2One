...
Le lot 19a n'avait fait que le retrait du code mort ; celui-ci solde le reste du périmètre du
lot 19 du plan directeur, plus les onze écarts que la recette des vagues 1 à 4 avait renvoyés.

### Ce qui est fait

**Raccourcis (spec §1.4).** `Views/Menus/MeetingShortcut.swift` est désormais la **seule**
déclaration des huit combinaisons : `⌘K` `⌘M` `⌘⇧A` `⌘⇧S` `⌘⇧N` `⌘⇧V` `⌘⏎` `⌃⌘F`. Chaque cas
porte son jeton, son libellé, sa touche, ses modificateurs, la surface qui le déclare (item de
menu ou vue, quand le geste a besoin d'un contexte que le menu n'a pas) et une note pour la
feuille d'aide. `MeetingCommands` y prend ses raccourcis au lieu de les épeler ; le `⌘⇧S` que
`CapturesStrip` redéclarait — troisième déclaration de la même combinaison — est retiré. Une
feuille « Raccourcis clavier… » s'ouvre depuis le menu `⋯` et **rend la table** : elle n'en
tient pas une seconde copie, et un test le vérifie.

`Tests/MeetingShortcutsTests.swift` (9 tests) tient quatre promesses : la table couvre la spec
et rien d'autre, aucune combinaison n'y figure deux fois, `MeetingCommands` la lit, et — le
vrai garde-fou — **aucun second déclarant** n'apparaît dans `OneToOne/**` sans être nommé. Le
test relit les sources, comme `ReviewStateTests` le fait déjà.

**Finitions renvoyées.** Neuf des onze écarts `(c)` de la recette :

| Écart | Ce qui a été fait |
|---|---|
| n° 3 — le titre de réunion n'est pas un titre | `EditableTextField` gagne un style `plain` et une fonte injectable. La cause était qu'un `NSViewRepresentable` **ignore** le `.font()` de l'environnement SwiftUI : le `.font(.plexSans(13, .semibold))` de la barre du haut n'avait aucun effet depuis quatre lots. `NSFont.plexSans` / `plexMono` complètent les jetons. Les 25 autres usages gardent, à la ligne près, la séquence historique |
| n° 11 — troncatures sans ellipsis | même cause racine : sans `usesSingleLineMode`, AppKit ignore `lineBreakMode`. Les cellules de la fiche projet en édition coupaient donc sans « … » |
| n° 5 — la bascule `Speakers` ne s'affiche jamais | elle attendait un segment déjà diarisé ; elle ne dépend plus que du mode de transcription (spec §2.4). Un **retrait** dans `MeetingView`, seul geste que ce fichier autorise |
| n° 6 — niveaux de risque en bleu et gris | deux tables divergeaient. `MeetingKPI.Level.teinte` devient la table unique : `report` pour critique et élevé, `warn` pour modéré, `ink/4` pour faible. Le bandeau, le rail Risques, la nav Relire et la fiche projet la lisent |
| n° 8 — `Citer` / `Envoyer` réservés à la pièce présentée | offerts par vignette (spec §4.1), avec le prédicat que le menu contextuel employait déjà. Il fallait mettre un document à l'écran pour pouvoir le citer |
| n° 9 — le point de statut disparaît en édition | AppKit ne rend pas de forme dans le `label:` d'un `Menu` ; le `Circle()` devient une `Image` |
| n° 10 — le pied de fiche n'existe qu'en édition | la mention de visibilité est rendue hors édition (spec §4.3) ; seuls `Annuler` / `Enregistrer` restent conditionnés |
| troncatures des vues | `lineLimit(1)` + `truncationMode(.tail)` sur onze libellés variables dans une largeur contrainte : onglets d'espace et du rail, noms de locuteurs et de collaborateurs, titre de groupe d'engagements, feuille des participants, chips de thèmes |
| `newTaskPomodoros` | retiré : sans lecteur depuis que le rail de 330 px a remplacé l'ancien panneau (lot 3). Les six assertions portent maintenant sur `newTaskEffortMinutes` |

**Outillage de recette.** `Scripts/recette-app.sh` refuse d'empaqueter un binaire plus vieux
que les sources (`--force` pour outrepasser), affiche `md5`, taille et horodatage de ce qu'il
copie, vérifie que la copie correspond à la source, et donne au bundle un
`CFBundleIdentifier` suffixé `.recette` avec le nom « OneToOne (recette) » — deux instances de
même identifiant se confondent dans les outils système, et c'est ainsi qu'une fenêtre de
production a été redimensionnée. `Scripts/recette-run.sh` refuse sur session verrouillée (la
clé `ioreg` est rendue **sans espaces** autour du `=` : la forme cherchée jusqu'ici ne
correspondait jamais) et pendant une réunion Teams ou Zoom, via le nouveau
`Scripts/window-titles.swift` (`CGWindowListCopyWindowInfo` ; jamais AppleScript, et en Swift
plutôt qu'en Python — `Quartz`/pyobjc n'est pas dans le python3 du système). Les codes d'écran
sont désormais **lus** dans `RecetteScreen.swift` au lieu d'être recopiés : la liste recopiée
ignorait déjà `5a`.

Les cinq vérifications de chaque script sont documentées dans son en-tête et ont été jouées :
binaire frais accepté, binaire touché refusé (code 1), `--force` accepté, identité `.recette`
lue dans le plist, verrou détecté, code d'écran inconnu rejeté, titres de fenêtres Teams lus
(« Calendar | APRIL | … », qui ne déclenche rien). **Aucune application graphique lancée.**

**Sauvegarde.** `BackupService` exporte enfin les huit tables du lot 0B qui manquaient :
notes horodatées, jalons et interlocuteurs de projet, et le domaine 1:1 entier. Les DTO sont
nichés là où l'objet appartient, donc aucune signature ne change et aucun appelant n'est
touché ; les quatre clés porteuses sont optionnelles, ce qu'un test vérifie en retirant les
clés d'un vrai export. Les références de réunion passent par `stableID` et sont recousues
après la restauration des réunions.

**Documentation.** `docs/architecture.md` §3, §5, §8, §9, §12 et §13 remis en accord avec le
code — il annonçait encore `SchemaV1` et 28 modèles, citait `Interview` supprimé en août,
décrivait sept onglets et 42 fichiers de tests. `docs/cleanup-report.md` §8 pour les retraits
et la dette. `CLAUDE.md` gagne une section « Écran de réunion (refonte 2026-09) » avec les
règles et les cinq pièges de la recette. ADR de bilan
`docs/adr/2026-09-08-refonte-ecran-reunion-bilan.md` : les douze décisions telles
qu'appliquées, treize écarts assumés avec les maquettes, les dettes. `docs/adr/README.md`
gagne l'index qui lui manquait.

### Ce qui est renvoyé, et pourquoi

- **La barre du haut en mode Relire** (écart n° 4). Vérification faite :
  `MeetingSpacesBar.estMasquee` est **correct** et déjà testé — il masque la barre
  d'**espaces** dans l'espace Réunion en Relire et la laisse dans Rapport et Ressources, qui
  n'ont pas de nav latérale. Ce que la recette a vu est la barre du **haut**, dont la
  disparition priverait de l'accès au type, au template et au `⋯` : arbitrage relevant de D0,
  pas une finition. **Rien n'a été touché.**
- **Le conflit `⌘⏎`.** Le menu Réunion l'emploie pour « Générer le rapport » et un menu natif
  l'emporte sur tout `keyboardShortcut` de vue, là où la spec §1.4 le veut pour valider un
  composeur. Changer l'un des deux est une décision produit. La table le documente, la feuille
  l'annonce, et `ActionComposer` continue de l'intercepter par `onKeyPress`.
- **Trois doublons de raccourci** (`⌘K`, `⌘M`, `⌃⌘F`) vivent dans
  `Views/Meeting/Session/**`, dossier tenu par le correctif #42. Chacun porte déjà son
  commentaire justificatif ; ils sont inscrits comme **exceptions nommées** dans le test, donc
  toute occurrence nouvelle le casse.
- **Le semis de locuteurs.** La bascule `Speakers` est corrigée, mais poser des locuteurs sur
  les quatre segments de `RefonteDemoSeed` — gelé pour les lots livrés — est une décision de
  semis.
...
- **La recapture des douze écrans** : aucun lancement d'application graphique dans ce lot.
...
de `docs/superpowers/specs/refonte-2026-09/`.

## Refonte de l'écran de réunion — état au 2026-09-08

Les comptes rendus de session des lots 0A à 19a ont été déplacés dans
`docs/superpowers/specs/refonte-2026-09/journal-des-lots.md` (4 460 lignes, vingt-neuf
sections, texte inchangé, ordre chronologique). Spécification, plan directeur et décisions
D0–D11 : `docs/superpowers/plans/2026-09-07-refonte-reunion-programme.md` §4 et §5. Bilan :
`docs/adr/2026-09-08-refonte-ecran-reunion-bilan.md`.

### Pile et ordre de fusion

Cinq PR partent de `master` et se fusionnent dans n'importe quel ordre ; les vingt suivantes
forment une chaîne linéaire, chacune basée sur la précédente. **Fusionner de haut en bas.**

| Ordre | PR | Branche | Lot |
|---|---|---|---|
| — | #18 | `docs/refonte-reunion-programme` | plan directeur et spécification (indépendant) |
| 1 | #20 | `feat/refonte-lot-0a-socle-visuel` | 0A — socle visuel, jetons, fontes Plex |
| 2 | #19 | `feat/refonte-lot-0b-socle-donnees` | 0B — schéma V3, tête de lecture, confidentialité |
| 3 | #21 | `feat/refonte-lot-1a-chrome` | 1a — barre du haut |
| 4 | #22 | `feat/refonte-lot-1b-espaces-kpi-assistant` | 1b — espaces, indicateurs, assistant |
| 5 | #23 | `feat/refonte-lot-2-notes-transcription` | 2 — notes ↔ transcription, frise audio |
| 6 | #24 | `feat/refonte-lot-3-rail-actions` | 3 — rail d'actions de 330 px |
| 7 | #27 | `feat/refonte-lot-4-mode-seance` | 4 — mode séance plein écran |
| 8 | #28 | `feat/refonte-lot-5-poste-pilotage` | 5 — poste de pilotage (mode Relire) |
| 9 | #30 | `feat/refonte-lot-6-ressources` | 6 — ressources, tiroir, épinglage |
| 10 | #26 | `feat/refonte-lot-10a-socle-1to1` | 10a — socle 1:1 |
| 11 | #29 | `feat/refonte-lot-10b-humeur-regles-recap` | 10b — humeur, objectifs, règles, récap |
| 12 | #25 | `feat/refonte-lot-9-fiche-projet` | 9 — fiche projet en panneau |
| 13 | #31 | `fix/refonte-1to1-window-crash` | correctif — ré-entrance Auto Layout |
| 14 | #32 | `feat/refonte-lot-16-atelier-socle` | 16 — atelier : socle des planches |
| 15 | #34 | `feat/refonte-lot-7-captures` | 7 — captures Teams / Zoom |
| 16 | #35 | `feat/refonte-lot-11-1to1-manager-seance` | 11 — 1:1 manager, séance |
| 17 | #33 | `feat/refonte-lot-12-1to1-manager-prepa` | 12 — 1:1 manager, préparation |
| 18 | #38 | `feat/refonte-lot-8-pastille` | 8 — pastille flottante |
| 19 | #39 | `feat/refonte-lot-13-1to1-collab-seance` | 13 — 1:1 collaborateur, séance |
| 20 | #40 | `feat/refonte-lot-15-rapport` | 15 — rapport, blocs optionnels, citations |
| 21 | #41 | `feat/refonte-lot-17-atelier-modes` | 17 — atelier : Schéma et Manuscrit |
| 22 | #36 | `fix/refonte-recette-vagues-1-4` | recette des vagues 1 à 4 |
| 23 | #43 | `feat/refonte-lot-14-1to1-collab-prepa` | 14 — 1:1 collaborateur, préparation |
| 24 | #42 | `fix/refonte-session-fullscreen-content` | correctif — le mode séance 1b devient atteignable |
...
Aucune ne bloque la fusion de la pile.

| Sujet | Question | Source |
|---|---|---|
| Lignes de démonstration dans le store de **production** | le lot 9 y a semé 1 projet `P25_110_1`, 1 réunion, 12 actions, 5 risques, 4 segments, 3 jalons, 3 interlocuteurs et 6 collaborateurs (dont « Laurent Deberti »). Les supprimer ? Rien n'a été touché : effacer des lignes d'un store de 32 Mo n'est pas une décision à prendre seul | lot 9 |
| Teinte de la barre de budget à 65,6 % | la règle chiffrée (vert) ou la maquette (orange) ? | lot 9 |
| Ordre de la pile d'avatars | l'ordre de la capture ou celui du modèle ? | lot 1 |
| Contraste `ok/deep` sur `ok/bg` | mesuré à 4,43:1, sous le seuil de 4,5 — assouplir la règle ou changer le jeton ? | lot 0A |
| Compteurs `Notes 6` et `EN ATTENTE 9` | la maquette les affiche, le semis ne les produit pas | lots 1, 12 |
| Bouton `Capture` en 1:1 | le reléguer dans `⋯`, comme le montre la maquette 2a ? | lot 11 |
| Barre du haut en mode Relire | la masquer, comme `1c`, prive du type, du template et du `⋯` — arbitrage relevant de D0 | recette, écart (c) n° 4 |
...
Table complète : `docs/cleanup-report.md` §8 et `docs/architecture.md` §13.

- ~3 000 lignes de code mort **hors** refonte, inventoriées par le lot 19a (22 fichiers) — un
  lot dédié, à arbitrer.
- `AppSettings.rightSidebarLayoutJSON` : colonne sans lecteur, en attente d'une version de
  schéma (supprimer une colonne casse la lightweight migration).
- Double vérité assumée `EngagementLedger` (dérivé) / `Commitment` (table) : convertir
  compterait deux fois.
- `ActionsViewMode.kanban` / `.sticky`, `MeetingSlidesPopover`, `CaptureSource.region` :
  atteignables ou orphelins, hors écran de réunion.
- Trois doublons de raccourci dans `Views/Meeting/Session/**`, inscrits comme exceptions
  nommées et défendus par un test.
- `Commitment.linkedAction` non sauvegardée ; `＋ Action` ne prend pas le clavier ;
  `MeetingView.swift` reste le plus gros fichier de `Views/` (~2 050 l.).
