...
/// d'émettre, 30 s d'absence avant de conclure à la fin — et, par construction,
/// jamais deux démarrages à moins de 35 s. Les abaisser est une décision, pas un détail.
...
struct TeamsCallObservationTests {
    ...
    @Test("Une absence brève après callStarted n'émet rien : ni fin d'appel, ni second démarrage")
    func briefAbsenceWhileStableEmitsNothing() {
        var state = TeamsCallState()
        _ = TeamsCallObservation.step(state: &state, input: input(true, "Réunion", 0))
        #expect(TeamsCallObservation.step(state: &state, input: input(true, "Réunion", 5)) == .callStarted)
        // La fenêtre disparaît brièvement puis revient : on reste en `stable`.
    ...
    @Test("Deux callStarted consécutifs sont séparés d'au moins 35 s : la fusion à 30 s est structurelle")
    func consecutiveStartsAreFarApart() {
        var state = TeamsCallState()
        _ = TeamsCallObservation.step(state: &state, input: input(true, "Réunion", 0))
        #expect(TeamsCallObservation.step(state: &state, input: input(true, "Réunion", 5)) == .callStarted)
        // Absence à partir de 6 s : la fin d'appel ne tombe qu'à 36 s.
        #expect(TeamsCallObservation.step(state: &state, input: input(false, "", 6)) == .none)
        #expect(TeamsCallObservation.step(state: &state, input: input(false, "", 35.9)) == .none)
        #expect(TeamsCallObservation.step(state: &state, input: input(false, "", 36)) == .callEnded)
        // Retour immédiat : il faut encore 5 s de stabilité avant un second démarrage.
        #expect(TeamsCallObservation.step(state: &state, input: input(true, "Réunion", 37)) == .none)
        #expect(TeamsCallObservation.step(state: &state, input: input(true, "Réunion", 41.9)) == .none)
        #expect(TeamsCallObservation.step(state: &state, input: input(true, "Réunion", 42)) == .callStarted)
    }
