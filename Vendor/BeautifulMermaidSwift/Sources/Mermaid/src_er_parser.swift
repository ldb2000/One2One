...
public enum ErParserError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .unsupportedLine(line):
            return "Unsupported ER diagram statement: '\(line)'."
        }
    }
}
...
private func _parseErDiagramEntry(_ lines: [String]) throws -> ErDiagram {
    ...
    for line in lines.dropFirst() {
        ...
        if let activeEntityId = currentEntityId {
            ...
            // Local patch (OneToOne): an unparseable attribute must fail the
            // native parse instead of being silently dropped.
            throw ErParserError.unsupportedLine(rawLine)
        }
        ...
        // Local patch (OneToOne): unconsumed statements (styles, directives…)
