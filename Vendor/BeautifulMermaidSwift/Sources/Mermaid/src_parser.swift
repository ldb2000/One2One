...
private enum _ParserEntryError: Error {
    ...
    // Local patch (OneToOne): unconsumed statements must fail the native
    // parse so callers can fall back to a fully compatible renderer.
    case unsupportedLine(String)
}
...
private func _parseFlowchart(_ lines: [String]) throws -> ParsedGraph {
    ...
    for line in lines.dropFirst() {
        ...
        try _parseEdgeLine(line, graph: &graph, subgraphStack: &subgraphStack)
    }
...
private func _parseStateDiagram(_ lines: [String]) throws -> ParsedGraph {
    ...
    for line in lines.dropFirst() {
        ...
        // Local patch (OneToOne): unconsumed statements (notes, concurrency
        // separators, directives…) must fail the native parse instead of
        // being silently dropped from the rendered diagram.
        throw _ParserEntryError.unsupportedLine(line)
    }
...
private func _parseEdgeLine(_ line: String, graph: inout _WorkingGraph, subgraphStack: inout [ParsedSubgraph]) throws {
    ...
    guard let firstGroup = _consumeNodeGroup(remaining, graph: &graph, subgraphStack: &subgraphStack), !firstGroup.ids.isEmpty else {
        // Local patch (OneToOne): a line that yields no node at all is an
        // unsupported statement, not a no-op.
        throw _ParserEntryError.unsupportedLine(line)
    }
    ...
    // Local patch (OneToOne): leftover text means part of the statement was
    // not understood (interaction syntax, malformed edge…). Silently
    // dropping it used to produce incomplete diagrams — and spurious nodes
    // (`click A callback` registered a node named "click").
