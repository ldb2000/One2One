...
// Local patch (OneToOne): unconsumed statements must fail the native parse
// so callers can fall back to a fully compatible renderer.
public enum XYChartParserError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .unsupportedLine(line):
            return "Unsupported xychart statement: '\(line)'."
        }
    }
}
...
public func parseXYChart(_ lines: [String]) throws -> XYChart {
    ...
    for line in lines {
        ...
        // parse instead of being silently dropped from the rendered chart.
        let trimmed = line.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.isEmpty || trimmed.hasPrefix("%%") {
            continue
        }
        throw XYChartParserError.unsupportedLine(line)
    }
