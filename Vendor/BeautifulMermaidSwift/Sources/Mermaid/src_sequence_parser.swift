...
public enum SequenceParserError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .unsupportedLine(line):
            return "Unsupported sequence diagram statement: '\(line)'."
        }
    }
}
...
private func _parseSequenceDiagramEntry(_ lines: [String]) throws -> SequenceDiagram {
    ...
    for rawLine in lines.dropFirst() {
        ...
        // Local patch (OneToOne): `autonumber`, standalone
        // `activate`/`deactivate`, `box`… are not rendered by this engine.
        // Silently dropping them used to produce incomplete diagrams; fail
        // instead so callers can fall back. Comments are the only lines a
        // caller may legitimately leave in.
        if line.hasPrefix("%%") {
            continue
        }
        throw SequenceParserError.unsupportedLine(line)
    }
