...
public enum ClassParserError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .unsupportedLine(line):
            return "Unsupported class diagram statement: '\(line)'."
        }
    }
}
...
private func _parseClassDiagramEntry(_ lines: [String]) throws -> ClassDiagram {
    ...
    for line in lines.dropFirst() {
        ...
        if let activeClassId = currentClassId, braceDepth > 0 {
            ...
            // Local patch (OneToOne): an unparseable member must fail the
            // native parse instead of being silently dropped.
            throw ClassParserError.unsupportedLine(rawLine)
        }
        ...
        // Local patch (OneToOne): unconsumed statements (notes, directives…)
        // must fail the native parse instead of being silently dropped.
        if rawLine.hasPrefix("%%") {
            continue
        }
        throw ClassParserError.unsupportedLine(rawLine)
